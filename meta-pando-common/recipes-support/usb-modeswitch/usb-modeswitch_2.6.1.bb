...
# Updated FILES to include systemd service files
FILES:${PN} = "${bindir} \
               ${sysconfdir} \
               ${libdir}/udev/usb_modeswitch \
               ${sbindir} \
...
do_install() {
    # Add UDEVDIR override to ensure correct installation path
    oe_runmake DESTDIR=${D} UDEVDIR=${D}${libdir}/udev install-statlink
