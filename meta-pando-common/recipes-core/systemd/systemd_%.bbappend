...
SRC_URI:append = " \
    ...
    file://journald-pando-os.conf \
    file://vacuum.conf \
    file://watchdog.conf \
    file://os.conf \
    file://65-pando-update-state.rules \
    file://10-zram.rules \
    file://zram-swap-init \
    file://dev-zram0.swap \
    file://pando_update_state_probe \
    file://pando-os-sysctl.conf \
...
do_install:append() {
    ...
    install -m 06444 ${WORKDIR}/journald-pando-os.conf ${D}/${sysconfdir}/systemd/journald.conf.d
    ...
    rm ${D}/usr/lib/systemd/system/sysinit.target.wants/systemd-journal-flush.service
    ...
    ### install -m 0755 ${WORKDIR}/pando_update_state_probe ${D}/usr/lib/udev/update_update_state_probe
    ### install -m 0755 ${WORKDIR}/zram-swap-init ${D}/usr/lib/udev/zram-swap-init

    # Move udev rules into /lib as /etc/udev/rules.d is bind mounted for custom rules
    mv ${D}/etc/udev/rules.d/*.rules ${D}/usr/lib/udev/rules.d/

    install -d -m 0755 ${D}/usr/lib/sysctl.d/
    install -m 0644 ${WORKDIR}/pando-os-sysctl.conf ${D}/usr/lib/sysctl.d/
...
FILES:udev += "\
    ...
    ${rootlibexecdir}/udev/rules.d/65-pando-update-state.rules \
    ${rootlibexecdir}/udev/pando_update_state_probe \
    ${rootlibexecdir}/udev/zram-swap-init           \
"

### RDEPENDS:${PN}:append = " os-helpers-fs balena-ntp-config util-linux periodic-vacuum-logs"
RDEPENDS:${PN}:append = " os-helpers-fs util-linux periodic-vacuum-logs"
