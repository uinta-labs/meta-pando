#
# Pando images customizations
#

ROOTFS_RO_UNNEEDED = "udev-hwdb"

inherit image_types_pando kernel-pando-noimage

# When building a Pando OS image, we also generate the kernel modules headers
# and ship them in the deploy directory for out-of-tree kernel modules build
DEPENDS += "coreutils-native jq-native ${@bb.utils.contains('PANDO_DISABLE_KERNEL_HEADERS', '1', '', 'kernel-devsrc kernel-headers-test', d)}"
...
deploy_image_license_manifest () {
    ...
    MACHINE_LICENSE_SUBDIR=`echo ${MACHINE} | tr '-' '_'`
    IMAGE_LICENSE_MANIFEST="${LICENSE_DIRECTORY}/${MACHINE_LICENSE_SUBDIR}/${IMAGE_NAME}/license.manifest"
...
hup_sanity_check() {
	...
	DATA_SECTORS=$(expr $(du --apparent-size -ks ${PANDO_BOOT_WORKDIR} | cut -f 1) \* 2)
	# Calculate fs overhead
	DIR_BYTES=$(expr $(find ${PANDO_BOOT_WORKDIR} | tail -n +2 | wc -l) \* 32)
	DIR_BYTES=$(expr $DIR_BYTES + $(expr $(find ${PANDO_BOOT_WORKDIR} -type d | tail -n +2 | wc -l) \* 32))
	FAT_BYTES=$(expr $DATA_SECTORS \* 4)
	FAT_BYTES=$(expr $FAT_BYTES + $(expr $(find ${PANDO_BOOT_WORKDIR} -type d | tail -n +2 | wc -l) \* 4))
	...
	LARGEST_FILE_SECTORS=$(expr $(find ${PANDO_BOOT_WORKDIR} -type f -exec du --apparent-size -k {} + | sort -n -r | head -n1 | cut -f1) \* 2)
	if [ -n "$LARGEST_FILE_SECTORS" ]; then
		TOTAL_SECTORS=$(expr $DATA_SECTORS \+ $FAT_OVERHEAD_SECTORS \+ $LARGEST_FILE_SECTORS)
		BOOT_SIZE_SECTORS=$(expr ${PANDO_BOOT_SIZE} \* 2)
		bbnote "pando-boot: FAT overhead $FAT_OVERHEAD_SECTORS sectors, data $DATA_SECTORS sectors, largest file $LARGEST_FILE_SECTORS sectors, boot size $BOOT_SIZE_SECTORS sectors."
		if [ $TOTAL_SECTORS -gt $BOOT_SIZE_SECTORS ]; then
			bbfatal "pando-boot: Not enough space for atomic copy operations."
		fi
	fi
}
...
do_pandoos_boot_dirgen_and_deploy () {
    ...
    for PANDO_BOOT_PARTITION_FILE in ${PANDO_BOOT_PARTITION_FILES}; do
        echo "Handling $PANDO_BOOT_PARTITION_FILE ."

        # Check for item format
        case $PANDO_BOOT_PARTITION_FILE in
            *:*) ;;
            *) bbfatal "Some items in PANDO_BOOT_PARTITION_FILES ($PANDO_BOOT_PARTITION_FILE) are not in the 'src:dst' format."
        esac

        # Compute src and dst
        src="$(echo ${PANDO_BOOT_PARTITION_FILE} | awk -F: '{print $1}')"
        if [ -z "${src}" ]; then
            bbfatal "An entry in PANDO_BOOT_PARTITION_FILES has no source. Entries need to be in the \"src:dst\" format where only \"dst\" is optional. Failed entry: \"$PANDO_BOOT_PARTITION_FILE\"."
        fi
        dst="$(echo ${PANDO_BOOT_PARTITION_FILE} | awk -F: '{print $2}')"
        if [ -z "${dst}" ]; then
        ...
        case $dst in
            /*)
                ...
                ;;
             *) bbfatal "$dst in PANDO_BOOT_PARTITION_FILES is not an absolute path."
        esac
        ...
        if [ -d "$src" ]; then
            ...
            bbfatal "$src is an invalid path referenced in PANDO_BOOT_PARTITION_FILES."
        fi
        ...
        for src in $sources; do
            ...
            directory=""
            for path_segment in $(echo ${PANDO_BOOT_WORKDIR}/${dst} | sed 's|/|\n|g' | head -n -1); do
            ...
            cp -rvfL $src ${PANDO_BOOT_WORKDIR}/$dst
            # If there is a signed version copy it too
            if [ -f "${src}.sig" ]; then
                cp -vfL "$src.sig" "${PANDO_BOOT_WORKDIR}/$dst.sig"
            fi
        done
    done
    echo "${IMAGE_NAME}" > ${PANDO_BOOT_WORKDIR}/image-version-info
    init_config_json ${PANDO_BOOT_WORKDIR}
    command -v init_board_config > /dev/null && init_board_config

    # Keep this after everything is ready in the pando-boot directory
    find ${PANDO_BOOT_WORKDIR} -xdev -type f \
        ! -name ${PANDO_FINGERPRINT_FILENAME}.${PANDO_FINGERPRINT_EXT} \
        ! -name config.json \
        -exec md5sum {} \; | sed "s#${PANDO_BOOT_WORKDIR}/##g" | \
        sort -k2 > ${PANDO_BOOT_WORKDIR}/${PANDO_FINGERPRINT_FILENAME}.${PANDO_FINGERPRINT_EXT}

    echo "Install pando-boot in the rootfs..."
    cp -rvf ${PANDO_BOOT_WORKDIR} ${IMAGE_ROOTFS}/${PANDO_BOOT_FS_LABEL}

    hup_sanity_check
}
do_pandoos_boot_dirgen_and_deploy[depends] += "${@bb.utils.contains_any('PANDO_IMAGE_BOOTLOADER', 'grub grub-efi', 'grub-conf:do_deploy', '', d)}"
...
add_image_flag_file () {
    echo "DO NOT REMOVE THIS FILE" > ${DEPLOY_DIR_IMAGE}/${PANDO_FLAG_FILE}
}

python pando_boot_sanity_handler() {
  kernel_file = d.getVar('KERNEL_IMAGETYPE', True) + d.getVar('KERNEL_INITRAMFS', True) + d.getVar('MACHINE', True) + '.bin'
  if kernel_file in d.getVar('PANDO_BOOT_PARTITION_FILES', True):
    bb.fatal("Pando OS only supports having the kernel in the root partition in rootfs/boot/KERNEL_IMAGETYPE. Please remove it from PANDO_BOOT_PARTITION_FILES.")
}

python pando_udev_rules_sanity_handler() {
...
python os_release_extra_data() {
    extra_data = []
    pando_board_rev = get_rel_path_rev(['meta-pando-common'], '../../../', d)
    if pando_board_rev == "unknown":
        bb.warn("Can't find board repository revision. This information will not be available in os-release.")
    meta_pando_rev = get_rel_path_rev(['meta-pando-common'], '../', d)
    if meta_pando_rev == "unknown":
        bb.warn("Can't find meta-pando repository revision. This information will not be available in os-release.")
    slug = get_slug(d)
    if slug == "unknown":
        bb.warn("Can't detect the slug. This information will not be available in os-release.")
    extra_data = [
        'PANDO_BOARD_REV="{0}"\n'.format(pando_board_rev),
        'META_PANDO_REV="{0}"\n'.format(meta_pando_rev),
        'SLUG="{0}"\n'.format(slug),
    ]
...
ROOTFS_POSTPROCESS_COMMAND += " \
    ...
    pando_root_quirks ; \
    pando_boot_sanity_handler ; \
    pando_udev_rules_sanity_handler ; \
    "
...
python do_image_size_check() {
    imgfile = d.getVar("PANDO_DOCKER_IMG")
    ext4file = d.getVar("PANDO_ROOTB_FS")
