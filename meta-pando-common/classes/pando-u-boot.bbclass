FILESEXTRAPATHS:append := ":${PANDO_COREBASE}/recipes-bsp/u-boot/files"
FILESEXTRAPATHS:append := ":${PANDO_COREBASE}/recipes-bsp/u-boot/patches"

INTEGRATION_NON_KCONFIG_PATCH = "file://pando-specific-env-integration-non-kconfig.patch"

# We require these uboot config options to be enabled for env_resin.h
SRC_URI += "file://pando_uboot.cfg"

SRC_URI += "${@bb.utils.contains('DISTRO_FEATURES', 'osdev-image', '', 'file://pando_uboot_prod.cfg', d)}"
SRC_URI += "${@bb.utils.contains('DISTRO_FEATURES', 'osdev-image', 'file://pando_uboot_delay.cfg', 'file://pando_uboot_nodelay.cfg', d)}"

# Machine independent patches
SRC_URI:append = " \
    file://env_resin.h \
    ${@bb.utils.contains('UBOOT_KCONFIG_SUPPORT', '1', '', '${INTEGRATION_NON_KCONFIG_PATCH}', d)} \
    file://pando_check_crc32.c \
    "
...
do_configure:append() {
    ...
    # we use the following sed to replace the static patch pando-specific-env-integration-kconfig.patch
...
PANDO_BOOT_PART = "1"
PANDO_DEFAULT_ROOT_PART = "2"
PANDO_ENV_FILE = "resinOS_uEnv.txt"
PANDO_EXTRA_ENV_FILE = "extra_uEnv.txt"
PANDO_UBOOT_DEVICES ?= "0 1 2"
PANDO_UBOOT_DEVICE_TYPES ?= "mmc"
PANDO_DEVICE_KERNEL_ADDR_VAR ?= "loadaddr"
PANDO_DEVICE_FDT_ADDR_VAR ?= "fdt_addr"
...
CONFIG_CMD_HASH = "1"
UBOOT_VARS = "PANDO_UBOOT_DEVICES \
              PANDO_UBOOT_DEVICE_TYPES \
              PANDO_BOOT_PART PANDO_DEFAULT_ROOT_PART \
              PANDO_DEVICE_KERNEL_ADDR_VAR \
              PANDO_DEVICE_FDT_ADDR_VAR \
              PANDO_IMAGE_FLAG_FILE \
              PANDO_FLASHER_FLAG_FILE \
              PANDO_ENV_FILE \
              PANDO_EXTRA_ENV_FILE \
              BASE_OS_CMDLINE \
              OS_BOOTCOUNT_FILE \
              OS_BOOTCOUNT_SKIP \
              OS_BOOTCOUNT_LIMIT \
...
do_inject_check_crc32_cmd() {
	...
	# upstream commit 09140113108 removes the cmd_tbl_t typedef
        if ! grep -q -r "cmd_tbl_t" ${S}/cmd/ ; then
            sed -i 's/cmd_tbl_t/struct cmd_tbl/g' ${WORKDIR}/pando_check_crc32.c
        fi
        # Older u-boot versions do not have env.h
        if [ ! -f ${S}/include/env.h ]; then
            sed -i 's/env.h/common.h/g' ${WORKDIR}/pando_check_crc32.c
            if ! grep -q env_get "${S}/include/common.h"; then
                sed -i 's/env_get/getenv/g' "${WORKDIR}/pando_check_crc32.c"
                sed -i 's/env_set/setenv/g' "${WORKDIR}/pando_check_crc32.c"
            fi
        fi
        cp ${WORKDIR}/pando_check_crc32.c ${S}/cmd/
        if ! grep -q "pando_check_crc32" ${S}/cmd/Makefile ; then
            cat >> ${S}/cmd/Makefile << EOF
ifndef CONFIG_SPL_BUILD
obj-y += pando_check_crc32.o
endif
EOF
        fi
    fi
